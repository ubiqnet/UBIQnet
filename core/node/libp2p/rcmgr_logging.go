	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				if len(errs) != 0 {
					n.logger.Errorf("Consider inspecting logs and raising the resource manager limits. Documentation: https://ubiqnet/blob/master/docs/config.md#swarmresourcemgr")
				}
