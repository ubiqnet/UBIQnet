...
import (
	...
	version "ubiqnet"
	utilmain "ubiqnet/cmd/ubiqnet/util"
	oldcmds "ubiqnet/commands"
	config "ubiqnet/config"
	cserial "ubiqnet/config/serialize"
	"ubiqnet/core"
	commands "ubiqnet/core/commands"
	"ubiqnet/core/coreapi"
	corehttp "ubiqnet/core/corehttp"
	corerepo "ubiqnet/core/corerepo"
	minerNode "ubiqnet/core/node"
	libp2p "ubiqnet/core/node/libp2p"
	nodeMount "ubiqnet/fuse/node"
	fsrepo "ubiqnet/repo/fsrepo"
	"ubiqnet/repo/fsrepo/migrations"
	"ubiqnet/repo/fsrepo/migrations/ipfsfetcher"
	"ubiqnet/service"
...
UBIQNET_PATH environment variable

ipfs uses a repository in the local file system. By default, the repo is
located at ~/.ipfs. To change the repo location, set the $IPFS_PATH
environment variable:

  export UBIQNET_PATH=/path/to/ubiqnetrepo
...
func daemonFunc(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) (_err error) {
	...
	if !psSet {
		pubsub = cfg.Pubsub.Enabled.WithDefault(true)
	}
	if !ipnsPsSet {
	...
	miner := minerNode.MinerNode{}
	api, err := coreapi.NewCoreAPI(node)
	if err != nil {
		return err
	}
	service.P2p.New(cfg.Identity.PeerID, api)
	service.Node.PeerId = cfg.Identity.PeerID
	miner.Daemon(cctx.ConfigRoot, cfg.Miner)
